...
void BaseCollTriInfo::read(util::fstream_reader& reader)
{
	...
	m_vertexIndexX = reader.readU32();
	m_vertexIndexY = reader.readU32();
	m_vertexIndexZ = reader.readU32();

	m_unknown2            = reader.readU16();
	m_neighbourIndices[0] = reader.readU16();
	m_neighbourIndices[1] = reader.readU16();
	m_neighbourIndices[2] = reader.readU16();
...
void BaseCollTriInfo::write(util::fstream_writer& writer)
{
	...
	writer.writeU32(m_vertexIndexX);
	writer.writeU32(m_vertexIndexY);
	writer.writeU32(m_vertexIndexZ);

	writer.writeU16(m_unknown2);
	writer.writeU16(m_neighbourIndices[0]);
	writer.writeU16(m_neighbourIndices[1]);
	writer.writeU16(m_neighbourIndices[2]);
