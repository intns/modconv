...
public:
	tokeniser() = default;
	inline tokeniser(const std::string& fileContents) { read(fileContents); }
	...
	inline std::optional<std::string> iterateUntilMatched(std::istringstream& iss, const std::string& lookingIn, char identifier)
	{
		if (std::count(lookingIn.begin(), lookingIn.end(), identifier)) {
			...
			while (identifiersPresent != 2) {
				if (!(iss >> lookingInCopy)) {
					return std::nullopt;
				}

				line += " " + lookingInCopy;
				identifiersPresent = static_cast<u32>(std::count(line.begin(), line.end(), identifier));
			}
